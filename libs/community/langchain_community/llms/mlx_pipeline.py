...
class MLXPipeline(LLM):
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        # END
        temp: Optional[float] = pipeline_kwargs.get("temp", 1.0)
        # Avoid division by zero
        if temp == 0.0:
            temp = 0.0000000001
        max_tokens: Optional[int] = pipeline_kwargs.get("max_tokens", None)
        verbose: bool = pipeline_kwargs.get("verbose", False)
        formatter: Optional[Callable] = pipeline_kwargs.get("formatter", None)

#TODO I fucked with this
        return generate(
            model=self.model,
            tokenizer=self.tokenizer,
            prompt=prompt,
...
class MLXPipeline(LLM):
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        temp: Optional[float] = pipeline_kwargs.get("temp", None)
        max_tokens: Optional[int] = pipeline_kwargs.get("max_tokens", None)
        verbose: bool = pipeline_kwargs.get("verbose", False)
        formatter: Optional[Callable] = pipeline_kwargs.get("formatter", None)
        repetition_penalty: Optional[float] = pipeline_kwargs.get(
